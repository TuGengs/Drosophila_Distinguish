 ...
 * @Description: ${创建一个视图区域}
 * @author ZhangJia --revise：DengWu
 * @date
 */
public class DragImageView extends ImageView {
	...
	private double testin[] = new double[22];
	private ArrayList<PointF> mArrayList = new ArrayList<PointF>();
	private StringBuffer mStringBuffer = new StringBuffer();
	private StringBuffer pointBuffer = new StringBuffer();
	private Context mContext;
	private boolean DRAW_POINT = true;
	...
	public OnDrawClickListener mOnDrawClickListener;

	interface OnDrawClickListener {
		public void onDrawChange(String result);
	};

	public void setOnDrawChangeListener(OnDrawClickListener mOnDrawClickListener) {
		this.mOnDrawClickListener = mOnDrawClickListener;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction() & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_UP:
				...
				if (Math.abs(current_x - start_x) < 3
						&& Math.abs(current_y - start_y) <3) {
					onTouchDown(event);
				}
				break;
	...
	void onTouchDown(MotionEvent event) {
		...
		if (!DRAW_POINT) { // 处理放缩
			...
			drawX = event.getX();
			drawY = event.getY();
			if (mArrayList.size() < 11) {
				mArrayList.add(new PointF(drawX, drawY));
				mStringBuffer.append("x = " + drawX + ";y = " + drawY + "\n");
				pointBuffer.append(drawX + "," + drawY + ",");
			} else {
				Toast.makeText(mContext, "超过了11个点!", Toast.LENGTH_LONG).show();
			}
			mOnDrawClickListener.onDrawChange(mStringBuffer.toString());
			// 通知onDraw()进行图片的绘制
			invalidate();
		}
	}
	...
	/**
	 * @Title: ${getTestin}
	 * @Description: ${返回11个点生成的数组}
	 * @param $
	 *            {}
	 * @return ${double[]}
	 * @throws
	 */
	public double[] getTestin() {
		if (pointBuffer.length() != 0) {
			pointBuffer.deleteCharAt(pointBuffer.length() - 1);
			String str[] = (pointBuffer.toString()).split(",");
			if (str.length < 22) {
				pointBuffer.append(",");
				return null;
			} else {
				...
				for (int i = 0; i < str.length; i++) {
					testin[i] = Double.parseDouble(str[i]);
				}
				return testin;
			}
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mArrayList.size() > 0) {
			for (PointF mPointF : mArrayList) {
				canvas.drawCircle(mPointF.x, mPointF.y, 8, mPaint);
			}
		}
	}

	public void changeDRAW_POINT(boolean dRAW_POINT) {
		DRAW_POINT = dRAW_POINT;
	}

	public boolean getDRAW_POINT() {
		return DRAW_POINT;
	}
